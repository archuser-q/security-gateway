...
import { Badge, Button, Card, Group, ScrollArea, Skeleton, Stack, Text, Tooltip } from '@mantine/core';
...
import { getRouteQueryOptions, getUpstreamQueryOptions } from '@/apis/hooks';
...
export const matchesSni = (host: string, sniPattern: string): boolean => {
...
const FlowBox = (props: { label: string; children: React.ReactNode }) => (
  <Card withBorder radius="md" p="sm" miw={170} style={{ flex: '1 1 0' }}>
...
// tóm tắt riêng). Chi tiết cấu hình từng plugin (trước đây nằm ngay dưới
// sơ đồ này) đã tách sang tab Plugins riêng - xem
// useRouteEffectivePlugins.ts cho phần logic gộp Route > Plugin Config >
// Service dùng chung giữa 2 nơi.
const RouteFlowDiagram = (props: {
  ...
  const {
    ...
    showPluginsBox,
  } = useRouteEffectivePlugins(route);
  ...
  return (
    <Card withBorder radius="md" p="md" mb="md">
      ...
      <ScrollArea type="auto" offsetScrollbars scrollbarSize={8}>
        <Group gap={0} wrap="nowrap" align="stretch" w="100%">
...
const RouteTlsSection = ({ route }: { route: APISIXType['Route'] | undefined }) => {
  ...
  // Route không đặt Host là cấu hình hợp lệ, khá phổ biến (route áp
  // dụng cho mọi domain, khớp thuần theo URI) - trước đây gặp trường
  // hợp này thì ẩn hẳn cả khối TLS, khiến người xem không rõ là do
  // route không liên quan gì tới SSL hay do lỗi/thiếu gì đó. Theo đúng
  // cách Traefik luôn hiện khối TLS kèm trạng thái rỗng "There is no
  // TLS configured" thay vì ẩn cả khối - giữ layout trang nhất quán,
  // tường minh hơn ẩn im lặng.
  if (hosts.length === 0) {
    return (
      <Card withBorder radius="md" p="md" mb="md">
